...
Quartz/remark can fail to parse these in some contexts.
Use:
  > [!note]
instead of:
  >[!note]
...
echo "OK: no '>[' callout markers found."
...
Quartz can mistake this for YAML frontmatter and fail the build.
Remove the leading "---" or replace it with valid frontmatter.
...
echo "OK: no leading standalone '---' before headings found."
